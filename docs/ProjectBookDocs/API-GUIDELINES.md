...
The API itself is **stateless and cookieless**: it authenticates every request via a
JWT bearer token and never sets a `Set-Cookie` header. All clients — browser or
programmatic — use the same mechanism.

### 2.1 JWT Bearer Tokens

**Flow:**
1. Client sends `POST /api/v1/auth/login` with email and password (and optional `remember`)
2. Server validates credentials against the goAuth engine and returns a JSON body:
   `{ access_token, refresh_token, access_expires_unix }` — no cookies are set
3. Client sends subsequent requests with `Authorization: Bearer <access_token>`
4. When the access token expires, the client calls `POST /api/v1/auth/refresh` with
   the refresh token to obtain a new access/refresh pair
5. Client sends `POST /api/v1/auth/logout` (bearer-authenticated) to revoke the
   underlying session

The access/refresh tokens should be stored securely by the client (secure storage on
mobile, httpOnly cookies for web — see 2.2 below).

### 2.2 Browser Clients (BFF Pattern)

The official ProjectBook web app does not talk to this API directly from the browser.
It runs a SvelteKit **Backend-for-Frontend (BFF)**: server-side routes call this API,
receive the JSON token pair from 2.1, and store them in httpOnly cookies scoped to the
web app's own origin. The browser only ever holds those BFF-issued cookies and never
sees the raw API tokens.

This cookie mechanism (names, expiry, refresh scheduling) is a **client-side concern
outside this API's contract** — it is documented in the web app's own docs
(`Web/src/lib/server/auth/cookies.ts`), not here. Any client is free to implement its
own token-custody strategy as long as it sends a valid `Authorization: Bearer` header.
...
Session lifetime is enforced server-side by the goAuth engine (see `docs/auth.md`
§5.1/5.3), not by any cookie. It depends on the `remember` flag sent at login:
...
| Session (default) | 7 days | `remember: false` (or omitted) |
| Session (remember me) | 30 days | `remember: true` — durable session via `LoginOptions.RememberMe` |

### 2.5 Notice Cookie

`projectbook_auth_notice` is a **web-client (BFF) cookie**, not something this API
sets. The SvelteKit web app uses it to pass transient messages between auth pages
(e.g., "Verification email sent"). It is not a security token and is out of scope for
API clients.
