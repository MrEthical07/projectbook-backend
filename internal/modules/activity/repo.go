...
func (r *repo) ListProjectActivity(ctx context.Context, projectID string, query listQuery) ([]map[string]any, error) {
...
func (r *repo) resolveProjectID(ctx context.Context, projectID string) (string, error) {
	...
	err := r.store.Execute(ctx, storage.RelationalQueryOne(
		`SELECT id::text FROM projects WHERE slug = $1 OR id::text = $1 LIMIT 1`,
